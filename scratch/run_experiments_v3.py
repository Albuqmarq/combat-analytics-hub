...
configs = [
    ("A: defaults", dict(n_estimators=200, max_depth=4, learning_rate=0.05, subsample=0.8, colsample_bytree=0.8)),
    ("B: deeper trees", dict(n_estimators=300, max_depth=6, learning_rate=0.03, subsample=0.8, colsample_bytree=0.7, min_child_weight=3)),
    ("C: conservative", dict(n_estimators=500, max_depth=3, learning_rate=0.01, subsample=0.9, colsample_bytree=0.9, reg_alpha=0.1, reg_lambda=2)),
    ("D: balanced", dict(n_estimators=300, max_depth=5, learning_rate=0.05, subsample=0.85, colsample_bytree=0.75, min_child_weight=5, gamma=0.1)),
    ("E: aggressive", dict(n_estimators=200, max_depth=5, learning_rate=0.1, subsample=0.7, colsample_bytree=0.6, min_child_weight=1)),
    ("F: regularized", dict(n_estimators=400, max_depth=4, learning_rate=0.03, subsample=0.8, colsample_bytree=0.8, reg_alpha=0.5, reg_lambda=3, gamma=0.3)),
]
...
for name, params in configs:
    clf = xgb.XGBClassifier(**params, early_stopping_rounds=15, random_state=42, eval_metric='logloss')
...
print(f"\nMelhor configuracao: {best_name} (Acc: {best_acc:.4f})")
...
print("\n--- RESUMO HISTORICO ---")
print(f"   v1 (6 deltas basicos):         Acc ~0.6138 | ROC-AUC ~0.6625")
print(f"   v2 (ablation, 9 deltas):       Acc ~0.6205 | ROC-AUC ~0.6607")
print(f"   v3 (26 features, tuned):       Acc {best_acc:.4f}")
